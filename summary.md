...
- `users` – Authentication (id, username, password_hash, membership_status, membership_expiry)  
...
- A membership system with tiered plans will be implemented.  
- **QRIS BCA** payment gateway integration via their API for processing subscriptions.  
- Payment verification webhook to update user's `membership_status` and `membership_expiry` in the database.
...
4. **Membership Page**  
   - Bootstrap-styled pricing tables.  
   - QRIS BCA payment integration with QR code display and payment status tracking.

5. **Profile Page**  
   - User account management with Bootstrap form components.  
   - Membership history and project statistics.
...
- Cron jobs for membership expiry checks and cleanup tasks.